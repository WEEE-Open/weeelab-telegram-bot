...
class Users:
    def __init__(
        self, admin_groups: List[str], tree: str, invite_tree: str, groups_tree: str
    ):
        ...
        with conn as c:
            ...
            if user is not None:
                try:
                    if user.need_update():
                        user.update(
                            c, self.admin_groups, self.excluded_groups, True, nickname
                        )
    ...
    def update_invite(
        self, invite_code: str, tgid: int, nickname: Optional[str], conn: LdapConnection
    ):
        ...
        with conn as c:
            result = c.search_s(
                self.invite_tree,
                ldap.SCOPE_SUBTREE,
                f"(inviteCode={invite_code_escaped})",
                (),
            )
            ...
            if len(result) > 1:
                raise DuplicateEntryError(
                    f"Invite code {invite_code} associated to {len(result)} invites"
                )
            ...
            if nickname is None:
                ...
                modlist.append(
                    (ldap.MOD_REPLACE, "telegramnickname", nickname.encode("UTF-8"))
                )
...
class People:
    ...
    def __sync(self, conn):
        ...
        for dn, attributes in result:
            dob = (
                self.schac_to_date(attributes["schacdateofbirth"][0].decode())
                if "schacdateofbirth" in attributes
                else None
            )
            dost = (
                self.schac_to_date(attributes["safetytestdate"][0].decode())
                if "safetytestdate" in attributes
                else None
            )
            person = Person(
                ...
                (
                    attributes["telegramnickname"][0].decode()
                    if "telegramnickname" in attributes
                    else None
                ),
                (
                    int(attributes["telegramid"][0].decode())
                    if "telegramid" in attributes
                    else None
                ),
                "haskey" in attributes and attributes["haskey"][0].decode() == "true",
                "signedsir" in attributes
                and attributes["signedsir"][0].decode() == "true",
    ...
    def schac_to_date(schac_date):
        return date(
            year=int(schac_date[:4]),
            month=int(schac_date[4:6]),
            day=int(schac_date[6:8]),
        )
...
class User:
    ...
    def update(
        ...
        if len(result) > 1:
            raise DuplicateEntryError(
                f"DN {self.dn} associated to {len(result)} entries (how!?)"
            )
        ...
        self.dateofsafetytest = (
            self._schac_to_date(attributes["safetytestdate"][0].decode())
            if "safetytestdate" in attributes
            else None
        )
        self.signedsir = (
    ...
    def search(
        ...
        return User(
            ...
            (
                People.schac_to_date(attributes["safetytestdate"][0].decode())
                if "safetytestdate" in attributes
                else None
            ),
            "signedsir" in attributes and attributes["signedsir"][0].decode() == "true",
    ...
    def __search_by_tgid(conn, tgid, tree) -> Tuple[Dict, str]:
        ...
        if len(result) > 1:
            raise DuplicateEntryError(
                f"Telegram ID {tgid} associated to {len(result)} entries"
            )
        dn, attributes = User.__extract_the_only_result(result)
        del result
        return attributes, dn
    ...
    def __search_by_nickname(conn, tgnick: str, tgid: int, tree) -> Tuple[Dict, str]:
        ...
        result = conn.search_s(
            tree,
            ldap.SCOPE_SUBTREE,
            f"(&(objectClass=weeeOpenPerson)(!(telegramId=*))(telegramNickname={tgnick}))",
            (),
        )
        if len(result) == 0:
            raise AccountNotFoundError()
        if len(result) > 1:
            raise DuplicateEntryError(
                f"Telegram nickname {tgnick} associated to {len(result)} entries"
            )
    ...
    def __update_id(dn: str, new_id: int, conn):
        conn.modify_s(
            dn, [(ldap.MOD_REPLACE, "telegramId", str(new_id).encode("UTF-8"))]
        )
